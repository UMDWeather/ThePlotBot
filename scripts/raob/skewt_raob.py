...
for r in range(len(wmoStaNum)):
  try:
    ...
    Ws = Ws[horder[::-1]]
    Wd = Wd[horder[::-1]]
    goodT = np.ma.where(T<100.)
    goodW = np.ma.where(Ws<900.)
    p = p[goodT] ; T = T[goodT]; Td = Td[goodT]
    Ws = Ws[goodW]; Wd = Wd[goodW]; hgt = hgt[goodW]
    ...
    Ws = Ws*units('m/s')
    Wd = Wd*units.deg
    Ws = Ws.to(units('knots'))
    u, v = get_wind_components(Ws,Wd)
    # plot data
    skew.plot(p, T, 'r')
    skew.plot(p, Td, 'g')
    H = 287.05 * (np.nanmean(T)+273.15) / 9.81
    Pw = [pmax/np.exp(hgt[z]/H) for z in range(len(hgt))]
    Pw = Pw * units.mbar
    Pw[-1] = p[0]
    skew.plot_barbs(Pw,u,v)
    skew.plot_dry_adiabats()
    skew.plot_moist_adiabats()
    skew.plot_mixing_lines()
    l = skew.ax.axvline(0, color='c', linestyle='--', linewidth=2) # freezing line
    h.plot_colormapped(u[::-1],v[::-1],Pw[::-1])
    #### calculate parameters
    # LCL
    l = lcl(p[0], T[0], Td[0])
    lcl_temp = dry_lapse(concatenate((p[0], l)), T[0])[-1].to(units.degC)
    ...
    plt.annotate('LFC: ',xy=(0.67,0.57),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('Surface CAPE: ',xy=(0.67,0.54),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('Mixed Layer CAPE: ',xy=(0.67,0.51),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('Most Unstable CAPE: ',xy=(0.67,0.48),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('Lifted Index: ',xy=(0.67,0.45),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('K Index: ',xy=(0.67,0.42),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('SWEAT Index: ',xy=(0.67,0.39),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('TT Index: ',xy=(0.67,0.36),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('0-3km Lapse Rate: ',xy=(0.67,0.33),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('3-6km Lapse Rate: ',xy=(0.67,0.30),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('0-3km SRH: ',xy=(0.67,0.27),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('Freezing Level: ',xy=(0.67,0.24),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('PBL Top: ',xy=(0.67,0.18),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    plt.annotate('Surface: '+str(round(p[0].magnitude,1))+'mb, '+str(round(T[0].magnitude,1))+'C',xy=(0.67,0.12),xycoords='figure fraction',horizontalalignment='left',verticalalignment='top',fontsize=12)
    # save figure
